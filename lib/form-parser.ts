...
export function parseFormConfig(input: unknown): FormConfig {
  ...
  if (!result.success) {
    throw new FormConfigError("Invalid form configuration", result.error.issues)
  }
...
const literalFromValue = (value: unknown): z.ZodLiteral<PrimitiveLiteral> | null => {
...
const applyStringConstraints = (schema: z.ZodString, field: FormField) => {
  ...
  if (validation?.pattern) {
    try {
      ...
      throw new FormConfigError(`Invalid regex pattern for field '${field.name}'`, [
        {
          code: z.ZodIssueCode.custom,
...
const applyNumberConstraints = (schema: z.ZodNumber, field: FormField): z.ZodTypeAny => {
  const { validation } = field
...
const buildLiteralUnion = (field: FormField) => {
  ...
  const literals = field.options
    ...
    .filter((literal): literal is z.ZodLiteral<PrimitiveLiteral> => literal !== null)
  ...
  return z.union(
    literals as [z.ZodLiteral<PrimitiveLiteral>, z.ZodLiteral<PrimitiveLiteral>, ...z.ZodLiteral<PrimitiveLiteral>[]],
  )
}
...
const buildFieldSchema = (field: FormField): z.ZodTypeAny => {
  ...
  const isRequired = requiredSetting === undefined ? true : Boolean(requiredSetting)
...
export function buildBackendValidator(config: FormConfig) {
  ...
  const fieldsWithSameAs = normalized.fields.filter((field) => field.validation?.sameAs)
